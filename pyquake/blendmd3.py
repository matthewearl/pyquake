...
def add_model(m: md3.MD3, anim_info: md3.AnimationInfo,
              pmove_frames: md3.PmoveFrames, obj_name: str, fps: float):
    ...
    # De-duplicate frames
    change_idxs = np.where(np.diff(pmove_frames.anim_idxs) != 0)[0]
    change_idxs = np.concatenate([[0], change_idxs + 1, [len(pmove_frames.anim_idxs) - 1]])
    start_times = pmove_frames.times[change_idxs[:-1]]
    end_times = pmove_frames.times[change_idxs[1:]]
    anim_idxs = pmove_frames.anim_idxs[change_idxs[:-1]]

    prev_anim_frame = None
    prev_blender_frame = None
    for anim_idx, start_time, end_time in zip(anim_idxs, start_times, end_times):
        anim_idx &= ~128    # This bit is flipped to force an anim restart, but it should otherwise be ignored.

        start_frame = int(start_time * fps)
        end_frame = int(end_time * fps)

        print(md3.PlayerAnimNumber(anim_idx), anim_idx, start_frame, start_time)
        ...
        for anim_frame, anim_frame_time in zip(anim_frames, anim_frame_times):
            ...
            print('blender frame', blender_frame, 'anim_frame', anim_frame)
